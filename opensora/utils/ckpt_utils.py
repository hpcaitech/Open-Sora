...
def load_checkpoint(model, ckpt_path, save_as_pt=True, ignore_state_dict_keys=[]):
    if ckpt_path.endswith(".pt") or ckpt_path.endswith(".pth"):
        state_dict = torch.load(ckpt_path, map_location=lambda storage, loc: storage)

        for key in ignore_state_dict_keys:
            # del state_dict["pos_embed_temporal"]
            del state_dict[key]
        ...
        # TODO: hasattr(model, 'module')? save as pt is already done in load_from_sharded_state_dict()
