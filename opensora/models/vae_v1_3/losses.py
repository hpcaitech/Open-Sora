...
class VAELoss(nn.Module):
    ...
    def forward(
        ...
        weighted_kl_loss = 0
        if self.kl_weight is not None and self.kl_weight > 0.0:
...
class GeneratorLoss(nn.Module):
    def __init__(self, disc_start=2001, disc_factor=1.0, disc_weight=0.5, **kwargs):
        super().__init__()
...
class DiscriminatorLoss(nn.Module):
    def __init__(self, disc_start=2001, disc_factor=1.0, disc_loss_type="hinge", **kwargs):
        super().__init__()

        assert disc_loss_type in ["hinge", "vanilla"]
        self.disc_factor = disc_factor
        self.disc_start = disc_start
