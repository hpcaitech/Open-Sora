...
class RotaryEmbedding(Module):
    ...
    def get_seq_pos(self, seq_len, device, dtype, offset=0, scale=None):
        interpolate_factor = self.interpolate_factor if scale is None else scale
        return (torch.arange(seq_len, device=device, dtype=dtype) + offset) / interpolate_factor

    def rotate_queries_or_keys(self, t, seq_dim=None, offset=0, freq_seq_len=None, scale=None):
        ...
        freqs = self.forward(
            self.get_seq_pos(seq_len, device=device, dtype=dtype, offset=offset, scale=scale),
            seq_len=seq_len,
            offset=offset,
        )
