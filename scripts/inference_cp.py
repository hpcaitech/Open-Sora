...
from opensora.acceleration.parallel_states import set_sequence_parallel_group, set_cond_groups
...
def sub_loop(cfg, pg: dist.ProcessGroup, device: str, dtype: torch.dtype):
    batch_size = cfg.get("batch_size", 1)
    num_sample = cfg.get("num_sample", 1)
    while True:
        metadata = [None] # z_shape, t_shape, additional_args, dtype
        dist.broadcast_object_list(metadata, 0, group=pg)
        z_shape, t_shape, additional_args = metadata[0], metadata[1], metadata[2], metadata[3]
        z = torch.empty(z_shape, device=device, dtype=dtype)
        t = torch.empty(t_shape, device=device, dtype=dtype)
        dist.broadcast(z, 0, group=pg)
        dist.broadcast(t, 0, group=pg)
...
def main():
    ...
    if is_distributed():
        ...
        enable_conditional_parallelism = True
        if enable_sequence_parallelism:
            set_sequence_parallel_group(dist.group.WORLD)
        if enable_conditional_parallelism:
            set_cond_groups()
    ...
    for i in progress_wrap(range(0, len(prompts), batch_size)):
        ...
        model_args["cp"] = enable_conditional_parallelism
