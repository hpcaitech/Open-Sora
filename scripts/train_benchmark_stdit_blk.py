...
from performance_evaluator import PerformanceEvaluator, profile
...
def main():
    ...
    x = torch.rand(cfg.batch_size, 4096, 1152).to(dtype=torch.bfloat16, device='musa')
    # y = torch.rand(cfg.batch_size, 1, 120, 1152).to(dtype=torch.bfloat16, device='musa')
    y = torch.rand(cfg.batch_size, 1, cfg.text_encoder.model_max_length, 1152).to(dtype=torch.bfloat16, device='musa')
    t = torch.rand(cfg.batch_size, 6912).to(dtype=torch.bfloat16, device='musa')
    t_tmp = torch.rand(cfg.batch_size, 3456).to(dtype=torch.bfloat16, device='musa')
    
    kwargs = {}
    kwargs['mask'] = torch.randint(1, (cfg.batch_size, cfg.text_encoder.model_max_length)).to(dtype=torch.int64, device='musa')
    kwargs['x_mask'] = torch.randint(1, (cfg.batch_size, 16)).to(dtype=torch.bool, device='musa')
    # kwargs['x_mask'] = kwargs['x_mask'].bool().to(device='musa')
    kwargs['t0'] = torch.rand(cfg.batch_size, 6912).to(dtype=torch.bfloat16, device='musa')
    kwargs['t0_tmp'] = torch.rand(cfg.batch_size, 3456).to(dtype=torch.bfloat16, device='musa')
    
    model_mem, act_mem = profile(name=cfg.model.type, 
            model = blk,
            x = x,
            t = t,
            y = y,
    ...
    print(f"model_mem {model_mem}; act_mem {act_mem}")
    ...
    #         x = torch.randn(cfg.batch_size, 3, cfg.dataset["num_frames"], cfg.dataset["image_size"][0], cfg.dataset["image_size"][1], dtype=dtype).to(device)  # (B, C, T, H, W)   
    #         model_args = dict()
    #         model_args['y'] = torch.randn(cfg.batch_size, 1, cfg.text_encoder['model_max_length'], text_encoder_output_dim).to(device, dtype)
    #         model_args['mask'] = torch.randn(cfg.batch_size, 1, cfg.text_encoder['model_max_length']).to(device)
    ...
    #             )
    #         performance_evaluator.end_iter(input_ids=torch.empty(cfg.batch_size, cfg.text_encoder['model_max_length']))
    #         performance_evaluator.start_new_iter()
...
# Cmd:
# torchrun --nnodes=1 --nproc_per_node=1 scripts/train_benchmark_stdit_blk.py configs/opensora-v1-1/train/16x256x256.py --data-path /home/dist/hpcai/duanjunwen/Open-Sora/dataset/panda_train/meta/meta_clips_caption_cleaned.csv
