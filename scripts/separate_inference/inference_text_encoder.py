...
def main():
    ...
    to_torch_dtype(cfg.get("dtype", "bf16"))
    ...
    if not os.path.exists(os.path.join(save_dir, date)):
        saved_idx = str(1).zfill(5)
    else:
        latest_idx = sorted([int(x) for x in os.listdir(os.path.join(save_dir, date))])[-1]
        saved_idx = str(latest_idx + 1).zfill(5)
    if is_main_process():
        os.makedirs(os.path.join(save_dir, date, saved_idx), exist_ok=True)
    ...
    for i in progress_wrap(range(0, len(prompts), batch_size)):
        ...
        for k in range(num_sample):
            ...
            for loop_i in range(loop):
                ...
                # == run text encoder ==
                caption_embs, emb_masks = text_encoder.t5.get_text_embeddings(batch_prompts_loop)
                caption_embs = caption_embs[:, None]
                if is_main_process():
                    torch.save(caption_embs, os.path.join(save_dir, date, saved_idx, f"{i}_{loop_i}_prompt.pt"))
