...
def run(rank, args, world_size, output_queue):
    if rank == 0:
        ...
        if os.getenv('DEBUG_ADDRESS') != None:
            import ptvsd; ptvsd.enable_attach(address=('localhost', int(os.getenv('DEBUG_ADDRESS'))), redirect_output=True); ptvsd.wait_for_attach(); print('waiting for debugger attachment')
        pass
    if rank != 0:
